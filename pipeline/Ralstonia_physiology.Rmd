...
df_summary <- df_DCW_PHB %>%
  ...
  mutate(
    g_DCW_L = case_when(condition %in% c("formate", "ammonium") ~ g_DCW_L/2, TRUE ~ g_DCW_L),
...
# preview table with flux to PHB during N-limitation
df_summary %>% filter(condition == "ammonium") %>%
  select(condition, dilution_rate, percent_PHB, P_g_PHB_g_DCW_h, P_mmol_PHB_g_DCW_h) %>%
  distinct %>% print
```
