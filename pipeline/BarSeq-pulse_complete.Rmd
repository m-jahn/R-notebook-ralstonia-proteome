...
What is the role of formate-essential genes? Are there patterns that emerge? We can submit the depleted gene list to STRING DB and retrieve a possible network of interactions.
...
df_str_formate_depl <- read_tsv(paste0("https://string-db.org/api/tsv/network?identifiers=", 
  list_formate_depleted, "&species=381666&required_score=400")) %>%
  mutate(across(matches("stringId"), function(x) gsub("381666.", "", x)))

# construct a new graph object from of pathway nodes/edges
# expected network:  number of nodes: = 34, number of edges = 74
as_tbl_graph(df_str_formate_depl)
