...
df_fitness_summary %>%
  ...
  xyplot(Norm_fg_median ~ Time | Substrate * factor(cluster), .,
    ...
    line_col = stdcol[as.numeric(factor(.[["Condition"]]))],
    xlab = "generations", ylab = "fitness",
    par.settings = custom.colorblind(), type = c("l"),
    between = list(x = 0.5, y = 0.5),
    scales = list(alternating = FALSE), lwd = 2,
    panel = function(x, y, groups, subscripts, line_col, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.text(2, 11, labels = paste("n =", sum(x == 0)))
...
What is the identity of the enriched genes? Which one are enriched in several conditions?
...
The mechanism for increased growth on fructose seems clearly different from the mechanism for faster growth in all other (mainly continous conditions).
