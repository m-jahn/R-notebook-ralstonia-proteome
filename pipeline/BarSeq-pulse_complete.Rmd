...
- FDH --> Cytochrome C reductase (bc1 complex III) --> Cytochrome c553 () --> Cytochrome C oxidase (cbb3 complex IV) --> O2
...
heatmap_fitness <- function(data, key = TRUE) {
  ...
  data %>% ungroup %>% arrange(gene_name) %>%
  mutate(gene_name = factor(gene_name, unique(gene_name))) %>%
  mutate(Condition = if_else(Condition == "pulse", "P", "C")) %>%
  mutate(cond = as.factor(paste0(Substrate, " - ", Condition))) %>%
  mutate(Norm_fg_median = replace(Norm_fg_median, Norm_fg_median < -6, -6)) %>%

  # plot heatmap
  levelplot(Norm_fg_median ~ gene_name * cond, .,
...
df_fitness_summary %>% filter(Time == 8, grepl("mo[aedbg]", gene_name)) %>%
  mutate(gene_name = stri_extract_first_regex(gene_name, "mo[aedbg]([A-Z0-9]+)?")) %>%
  heatmap_fitness(key = FALSE) %>% print(position = c(0, 0.5, 0.5, 1), more = TRUE)

df_fitness_summary %>% filter(Time == 8, grepl("fd[swho]", gene_name)) %>%
  ...
  heatmap_fitness(key = FALSE) %>% print(position = c(0.45, 0.49, 1, 1), more = TRUE)

df_fitness_summary %>% filter(Time == 8, locus_tag %in% c("H16_A3451", "H16_A0830",
    "H16_A0846", "16_B1452", "H16_A3576", "H16_A1121", "H16_A1120", "H16_A1031") 
    | grepl("qcr|cco|cta|cox[MNOPQ] ", gene_name)
    ) %>%
  mutate(gene_name = stri_extract_first_regex(gene_name, "H16_[AB][0-9]+|(qcr|cco|cta|cox)([A-Z0-9]+)?")) %>%
  heatmap_fitness(key = FALSE) %>% print(position = c(0, 0, 0.69, 0.56), more = TRUE)
```
