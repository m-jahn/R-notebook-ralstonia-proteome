...
plot_cbb_fitness_bee <- df_fitness_summary %>% ungroup %>%
  filter(time == 8, str_detect(gene_name, "cbb.|cfx.|cbx.")) %>%
  mutate(gene_name = str_replace(gene_name, "cfx|cbx", "cbb")) %>%
  mutate(condition = paste0(substrate, "\n", toupper(substr(condition, 1, 1)))) %>%
  
  xyplot(norm_gene_fitness_median ~ factor(condition), .,
    groups = condition, par.settings = custom.colorblind(col = subs_col),
    labels = stri_extract_first(.[["gene_name"]], regex = "cbb.|cfx.|cbx."),
    xlab = "", ylab = "fitness (8 gen)", ylim = c(-7.5, 2.5),
    scales = list(alternating = FALSE),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.abline(h = 0, lwd = 1.5, col = grey(0.7), lty = 2)
      coords <- panel.beeswarm(x, y, bin_y = TRUE, breaks_y = seq(-10, 4, 0.5), 
        return_coords = TRUE, ...)
      panel.directlabel(x = coords$x, y = coords$y, y_boundary = c(-10, -4), 
        cex = 0.6, draw_box = TRUE, box_line = TRUE, ...)
    }
  )
...
plot_cbb_fitness_select <- df_fitness %>% ungroup %>%
  ...
  filter(str_detect(gene_name, "(cbb|cfx|cbx)[PSLZKGATE]P|cbbR")) %>%
  ...
  xyplot(norm_gene_fitness ~ factor(time) | gene_name, .,
    groups = condition, par.settings = custom.lattice(col = subs_col),
    type = c("p", "l"), xlab = "generations", ylab = "fitness",
    ylim = c(-4, 4), layout = c(5, 2), between = list(x = 0.5, y = 0.5),
...
```{r, include = FALSE, fig.width = 5.5, fig.height = 6}
svg("../figures/figure_fitness_cbb.svg", width = 5.5, height = 6)
print(plot_cbb_fitness_bee, position = c(0, 0.53, 1, 1), more = TRUE)
print(plot_cbb_fitness_select, position = c(0, 0, 1, 0.6))
dev.off()
```
...
```{r, message = FALSE}
df_ccm_enzymes <- list(
  `Calvin cycle` = c("PGK", "TPI", "GAPD", "FBA", "FBP", "TKT1", "TKT2", "TAh", "RPE", "RPI", "PRUK", "RBPC", "FDH"),
  `Entner Douderoff` = c("PGI", "G6PDH2r", "PGL", "EDD", "EDA"),
...
plot_ccm_bee <- df_fitness_summary %>%
  ...
  ungroup %>% filter(time == 8) %>%
  mutate(condition = paste0(substrate, "\n", toupper(substr(condition, 1, 1)))) %>%
  mutate(gene_name = stri_extract_first(gene_name, regex = "[a-zA-Z0-9]+")) %>%

  xyplot(norm_gene_fitness_median ~ factor(condition) | model_group_short, .,
    groups = condition, par.settings = custom.colorblind(col = subs_col),
    labels = .[["gene_name"]], xlab = "", ylab = "fitness (8 gen)", 
    ylim = c(-7.5, 2.5), as.table = TRUE, between = list(x = 0.5, y = 0.5),
    scales = list(alternating = FALSE),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.abline(h = 0, lwd = 1.5, col = grey(0.7), lty = 2)
      coords <- panel.beeswarm(x, y, bin_y = TRUE, breaks_y = seq(-10, 4, 0.4),
        return_coords = TRUE, ...)
      panel.directlabel(coords$x, coords$y, y_boundary = c(-10, -3), cex = 0.6,
...
plot_ccm_fitness_select <- df_fitness %>% ungroup %>%
  left_join(select(df_ref, locus_tag, gene_name), by = "locus_tag") %>%
  inner_join(df_model_reactions, by = "locus_tag") %>%
  filter(str_detect(gene_name, "fds|pgl|pgi|edd|eda|ppc|pdh|mae")) %>%
  mutate(condition = paste0(substrate, " - ", toupper(substr(condition, 1, 1)))) %>%
  mutate(gene_name = stri_extract_first(gene_name, regex = "[a-zA-Z0-9]+")) %>%
