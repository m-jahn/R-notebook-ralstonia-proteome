...
df_pool <- df_pool %>% #group_by(scaffold, pos) %>%
  filter(nTot > 1) # & length(pos) <= 5)) %>% ungroup
```
...
df_essential <- df_pool_annotated %>%
  ...
  # due to low tn_interval insertion frequency (below 30 per 10 kb (~ 3 tn per gene on average)
  #mutate(
  #  essentiality = case_when(
  #    tn_interval <= 10 & essentiality == "essential" ~ "ambiguous",
  #    TRUE ~ essentiality)
  #) %>%
...
print(plots_cbb_mp[[1]], position = c(0,0,1,0.5))
```
Example for several subunits of *gat* operon, the Glutamyl/Aspartyl-tRNA(Gln/Asp) amidotransferase, responsibe for 'charging' glutaminyl-tRNAs (actually transferring an amino group to a glutamyl-tRNA). Essential.
