...
### Predicitng essentiality from insertion index distribution
...
## Comparison of essentiality with (change in) protein abundance

Essentiality can also be correlated to the MS protein abundance measurements, or the change in protein abundance over conditions.
An advanced analysis is also to correlate essentiality or fitness to utilization. First we need to import protein measurements.

```{r}
load("../data/input/Ralstonia_eutropha.Rdata")
df_prot <- Ralstonia_eutropha; rm(Ralstonia_eutropha)

df_prot <- df_prot %>% left_join(select(df_essential, locus_tag,
  insertion_index, insertion_probability, essentiality))
```
...
plot_ess_vs_abund <- df_prot %>% group_by(essentiality, locus_tag) %>%
  summarize(average_mass_g_gDCW = mean(mean_mass_fraction)*0.68) %>%
  xyplot(log10(average_mass_g_gDCW) ~ essentiality %>% factor(., unique(.)[c(2,1,3)]), .,
    ...
    panel = function(x, y, ...) {
      ...
      panel.text(1:3, -7.3, labels = paste0("n=", table(x)), cex = 0.6)
    }
  )

# CV (relative SD) of protein abundance per essentiality group
plot_ess_vs_change <- df_prot %>% group_by(essentiality, locus_tag) %>%
  summarize(change_mass_g_gDCW = sd(mean_mass_fraction)/mean(mean_mass_fraction)) %>%
  xyplot(log10(change_mass_g_gDCW) ~ essentiality %>% factor(., unique(.)[c(2,1,3)]), .,
    par.settings = boxplot_colorblind,
    horizontal = FALSE, do.out = FALSE,
    xlab = "", ylab = expression("CV m"[protein]*" [log"[10]*"]"),
    scales = list(alternating = FALSE, x = list(rot = 25, cex = 0.7)),
    between = list(x = 0.5, y = 0.5),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.violin(x, y, lwd = 0, col = grey(0.6, 0.5), box.width = 0.7, ...)
      panel.bwplot(x, y, pch = "|", box.width = 0.3, ...)
      panel.pvalue(x, y, cex = 0.7, fixed_pos = 0.9, verbose = TRUE, ...)
      panel.text(1:3, -1.05, labels = paste0("n=", table(x)), cex = 0.6)
    }
  )

print(plot_ess_vs_abund, position = c(0,0,0.5,1), more = TRUE)
print(plot_ess_vs_change, position = c(0.5,0,1,1))
```
...
df_essential %>% filter(!is.na(model_essential)) %>%
...
Export selected figures to `*.svg` image files.
