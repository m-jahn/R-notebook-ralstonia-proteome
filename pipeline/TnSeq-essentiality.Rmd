...
df_pool_annotated <- foverlaps(dt_pool, dt_ref, by.x = c("scaffold", "begin", "end"), type = "within") %>%
  as_tibble %>%
  select(barcode, rcbarcode, nTot, scaffold, pos, begin,
...
df_pool_annotated <- df_pool_annotated %>%
  ...
  mutate(tn_per_gene = length(barcode)) %>%
...
df_pool_annotated <- df_pool_annotated %>%
  ...
  mutate(length_interval = cut_interval(pos, length = 10000)) %>%
  ...
  mutate(tn_interval = zoo::rollapply(central,
    ...
    width = length_interval, fill = NA)) %>%
  
  # finally determine insertion index
  group_by(old_locus_tag) %>%
  mutate(insertion_index = median(na.rm = TRUE,
    (sum(central, na.rm = TRUE) / gene_length) / (tn_interval / 10000)
  ) %>% replace_na(0) %>% replace(., . > 200, NA))

# plot distribution of insertion index
df_pool_annotated %>%
...
df_essential <- df_pool_annotated %>%
  ...
  summarize(
    ...
    n_barcodes_central = sum(central),
    scaffold = scaffold[1],
    begin = begin[1],
    end = end[1],
  ...
  mutate(
    essentiality = case_when(
      ...
      between(insertion_index, essential$lower_t, essential$upper_t) ~ "ambiguous"
    )
  ) %>%
...
chromos = "NC_005241.1" # "NC_008314.1" #
cbb_start <- 435000 # 1542000 # 
cbb_end <- 450000 # 1568000 # 

# plot CBB operon on chromosome 2
plot_cbb_locus <- df_essential %>%
  mutate(gene_name = eggNOG_name, essentiality = factor(essentiality)) %>%
  filter(start > cbb_start, start < cbb_end, scaffold == chromos) %>%
  
  xyplot(end ~ start, .,
    groups = essentiality, cex = 0.6, lwd = 1,
    par.settings = custom.colorblind(),
    scales = list(draw = FALSE),
    xlim = c(cbb_start, cbb_end),
...
plot_cbb_tns <- df_pool_annotated %>%
  filter(begin > cbb_start, begin < cbb_end, scaffold == chromos) %>%
  
  xyplot(rep_along(pos, 1) ~ pos, .,
    cex = 3, pch = "|",
    par.settings = custom.colorblind(),
    scales = list(draw = FALSE),
    xlim = c(cbb_start, cbb_end),
...
plot_cbb_reads <- df_pool_annotated %>%
  filter(begin > cbb_start, begin < cbb_end, scaffold == chromos) %>%
...
print(plot_cbb_reads, position = c(0,0.65,1,1), more = TRUE)
print(plot_cbb_tns, position = c(0,0.4,1,0.75), more = TRUE)
print(plot_cbb_locus, position = c(0,0,1,0.5))
```
