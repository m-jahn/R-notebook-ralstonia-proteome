...
The protein allocation for the two low-abundant machines, replication and transcription is very hard to see. We can update the Y-axis limits for this plot and replot it with 10-fold 'zoom'. Abundance of replication machinery increases with growth rate from 0.0010 to a maximum of 0.0015 g/gDCW, an increase by 20-50%. For transcription, abundance increases with growth rate from 0.006 to 0.008 g/gDCW, an increase of 33%. 
...
plot_machinery <- df_machinery %>%
  ...
  xyplot(mass_g_gDCW + predicted_mass_g_gDCW ~ factor(growth_rate) | 
    ...
    scales = list(alternating = FALSE, x = list(at = c(2,4))), as.table = TRUE,
...
print(plot_machinery, position = c(0,0,0.5,1), more = TRUE)
print(update(plot_machinery, ylim = c(-0.001, 0.012)), position = c(0.5,0,1,1))
```
...
```{r, fig.width = 6.0, fig.height = 4.8, message = FALSE}
...
print(plot_machinery, position = c(0, 0, 0.82, 1.027), more = TRUE)
print(plot_machinery_util, position = c(0.79, 0, 1, 1.))
grid::grid.text(c("A", "B"), x = c(0.02, 0.79), y = c(0.97,0.97))
```

```{r, include = FALSE}
svg("../figures/figure_machine_util.svg", width = 6.0, height = 4.8)
print(plot_machinery, position = c(0, 0, 0.82, 1.027), more = TRUE)
print(plot_machinery_util, position = c(0.79, 0, 1, 1.))
grid::grid.text(c("A", "B"), x = c(0.02, 0.79), y = c(0.97,0.97))
dev.off()
...
cbb_selected <- c("PGK", "GAPD", "FBA", "FBP", "TKT1", "PRUK", "RBPC") # "TKT2", "TAh", "RPE", "RPI"
...
plot_calvin <- df_prot_comp %>%
  ...
  xyplot(`mass [g/gDCW]` + `predicted mass [g/gDCW]` ~ factor(growth_rate) | 
    ...
    par.settings = custom.colorblind(), auto.key = list(columns = 2, cex = 0.7),
    xlab = expression("µ [h"^-1*"]"), ylim = c(-0.002, 0.032),
    ylab = expression("m"[protein]*" [g gDCW"^-1*"]"),
    scales = list(alternating = FALSE, x = list(at = c(2,4)),
      y = list(at = c(0.01, 0.02, 0.03))), as.table = TRUE,
...
plot_calvin_util <- df_prot_comp %>%
  ...
  xyplot(percent_utilization*100 ~ factor(growth_rate) | enzyme * substrate, .,
    par.settings = custom.colorblind(), ylim = c(0, 100),
    scales = list(alternating = FALSE, x = list(at = c(2,4))),
    xlab = expression("µ [h"^-1*"]"), ylab = "",
    as.table = TRUE, between = list(x = 0.5, y = 0.5),
...
print(plot_calvin, position = c(0, 0, 0.83, 1.027), more = TRUE)
...
enzyme_selection <- c("CO2t", "EDD", "CS", "O2t")

plot_mixo_enz <- xyplot(abs(value) ~ as.factor(CO2_refixation) | 
    factor(key, unique(key)), aspect = 0.9,
  filter(df_mixflux, key %in% enzyme_selection, carbon_conc == 0.655),
  par.settings = custom.colorblind(),
...
```{r, fig.width = 4.5, fig.height = 5.5, include = FALSE}
svg("../figures/figure_refixation.svg", width = 4.5, height = 5.5)
print(plot_mixo_mu[[1]], position = c(0,0.57,0.53,1), more = TRUE)
print(plot_mixo_mu[[2]], position = c(0.48,0.57,1,1), more = TRUE)
print(plot_mixo_enz, position = c(0.04, 0, 0.66, 0.62))
