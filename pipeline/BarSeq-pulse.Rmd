...
To measure depletion for more than 2 time points gives more confidence that the trends we see are real and consistent over several generations. Another question is to decide how many generations are enough to determine significant differences in fitness. It seems that a second time point such as 16 generations helps to judge if fitness effects are consistent or not. If several time points are available, one can also fit a linear model and compare ANOVA p values and R^2. We can plot a subset of genes, e.g. with the highest differential ftiness between two selected conditions.
...
# look at genes with differential fitness
df_diff_fitness %>% filter(SP_LP) %>%
  inner_join(df_fitness) %>%
  filter(Condition != "continuous") %>%
  draw_fitness_ts
```
...
plot_cbb_fitness_bee <- df_fitness_summary %>%
  filter(Time == 16, str_detect(gene_name, "cbb.|cfx.|cbx.")) %>%
  draw_beeswarm
...
print(plot_cbb_fitness_bee, position = c(0.3,0.54,1,1), more = TRUE)
print(plot_cbb_fitness_select, position = c(0,0,1,0.62))
```
```{r, include = FALSE}
svg("../figures/figure_fitness_cbb.svg", width = 5.5, height = 5.5)
print(plot_cbb_fitness_bee, position = c(0.3,0.54,1,1), more = TRUE)
print(plot_cbb_fitness_select, position = c(0,0,1,0.62))
dev.off()
```
...
Particularly, this is the condition `continuous` in the BarSeq data set (t = 8 or 16 generations), that is identical to the condition `FRC 0.1` in MS data (chemostat, fixed growth rate 0.1/h, 0.5 g/L fructose, 1 g/L NH4). We can import the MS proteomics data and see if (change in) fitness is correlated to protein abundance.
