...
plot_cbb_fitness_bee <- df_fitness_summary %>% ungroup %>%
  ...
  xyplot(norm_gene_fitness_median ~ factor(condition), .,
    groups = factor(condition), par.settings = custom.colorblind(col = subs_col),
    ...
    panel = function(x, y, ...) {
      ...
      panel.key(labels = rep(c("formate", "fructose", "succinate"), each = 2) %>% 
        paste0(" - ", c("C", "P")), cex = 0.75, corner = c(0.98, 0.05))
    }
  )
...
plot_cbb_fitness_select <- df_fitness %>% ungroup %>%
  ...
  filter(str_detect(gene_name, "cbbR|cbbPP|cbbSP|cbbL")) %>%
  ...
  xyplot(norm_gene_fitness ~ factor(time) | gene_name, .,
    ...
    ylim = c(-7.9, 2.5), layout = c(2, 2), between = list(x = 0.5, y = 0.5),
    scales = list(alternating = FALSE), lwd = 2, as.table = TRUE,
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
...
print(plot_cbb_fitness_bee, position = c(0, -0.02, 0.64, 1), more = TRUE)
print(plot_cbb_fitness_select, position = c(0.6, 0, 1, 1))
dev.off()
```
...
```{r, fig.width = 7.7, fig.height = 5.2}
plot_ccm_bee <- df_fitness_summary %>%
  ...
  xyplot(norm_gene_fitness_median ~ factor(condition) | model_group_short, .,
    ...
    panel = function(x, y, ...) {
      ...
      panel.text(6, -3.5, labels = "significant", col = grey(0.6), cex = 0.65)
      coords <- panel.beeswarm(x, y, bin_y = TRUE, breaks_y = seq(-10, 4, 0.4),
        return_coords = TRUE, ...)
...
```{r, include = FALSE}
svg("../figures/figure_fitness_ccm.svg", width = 7.7, height = 5)
print(plot_ccm_bee)
dev.off()
```
