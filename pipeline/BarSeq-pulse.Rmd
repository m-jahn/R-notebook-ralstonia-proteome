...
df_fitness %>% inner_join(
  ...
  xyplot(Norm_fg ~ factor(Time) | str_extract(gene_name, "[a-zA-Z0-9_]+"), .,
...
And finally investigate their function, or at least enrichment in certain functional groups. We can plot fitness scores per functional group to see if there's a trend.

```{r, message = FALSE}
# reformat fitness data a bit for sole purpose of plotting
df_heatmaps <- df_diff_fitness %>% 
  select(-COG_Process) %>%
  mutate(delta_LP_CO = continuous-`long pulse`, delta_SP_CO = continuous-`short pulse`, 
    delta_SP_LP = `long pulse`-`short pulse`) %>%
  pivot_longer(names_to = "comparison", values_to = "delta_fitness", delta_LP_CO:delta_SP_LP) %>%
  left_join(df_ref) %>%
  mutate(
    gene_name = gene_name %>% str_extract("[a-zA-Z0-9_]+"),
    comparison = comparison %>% gsub("delta_", "", .)
  )

# differential fitness for genes with enrichment 
df_heatmaps %>% filter(SP_CO & LP_CO & !SP_LP) %>%
  filter(COG_Process == "Transcription")
```

The largest group is transcription which is interesting. It seems that some **transcription factors** within this set are important for adaptation to changing conditions, cell cycling, and so on.

**6 TFs have high fitness in pulse conditions**
...
**4 TFs have low fitness in pulse conditions**
...
This is a test run to investigate if filtering genes by a simple fitness threshold instead of the differential fitness, dF, gives more comprehensive results. For each sample lower and upper fitness threshold are defined (or t-statistics) and all genes are plotted with fitness score on a heatmap.
