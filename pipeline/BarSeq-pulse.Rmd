---
title: "Pipeline for competition experiments with a barcoded transposon library"
...
plot_fitness_comp <- lapply(
  ...
  xyplot(get(var[1]) ~ get(var[2]),
    df_fitness_comp, groups = tstat_median < -3,#abs(get(var[1])-get(var[2])) > 2,
...
And finally investigate their function, or at least enrichment in certain functional groups.

```{r}
df_test <- filter(df_diff_fitness, SP_CO & LP_CO & !SP_LP) %>% select(-COG_Process) %>%
  inner_join(df_ref) %>%
