...
Here, we will apply the **top-down approach**, first select known groups of interest, and then compare average fitness score for the three conditions. We start with one of the most itneresting features of *Ralstonia*, the *cbb* operon.
...
plot_cbb_fitness_bee <- df_fitness_summary %>%
  ...
  xyplot(Norm_fg_median ~ factor(Condition, unique(Condition)[c(1,3,2)]), .,
    groups = seq_type,
    xlab = "", ylab = "fitness", ylim = c(-1.25, 1.25),
...
plot_cbb_fitness_time <- df_fitness %>% filter(str_detect(gene_name, "cbb.|cfx.")) %>%
  ...
  xyplot(Norm_fg ~ factor(Time) | str_extract(gene_name, "[a-zA-Z0-9_]+"), .,
    ...
    par.settings = custom.colorblind(), type = c("p", "l"),
    xlab = "generations", ylab = "fitness",
    pch = 19, between = list(x = 0.5, y = 0.5), ylim = c(-5, 5),
    scales = list(alternating = FALSE), lwd = 2,
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.errbars(x, y, ewidth = 0, ...)
...
```
```{r, include = FALSE}
svg("../figures/figure_fitness_cbb.svg", width = 4, height = 7.5, )
print(plot_cbb_fitness_bee, position = c(0,0.65,1,1), more = TRUE)
print(plot_cbb_fitness_time, position = c(0,0,1,0.7))
dev.off()
```
